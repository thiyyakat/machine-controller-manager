...
func (c *controller) manageAutoPreservationOfFailedMachines(ctx context.Context, machines []*v1alpha1.Machine, machineSet *v1alpha1.MachineSet) []*v1alpha1.Machine {
	if machineSet.Annotations[machineutils.AutoPreservationDisabledAnnotationKey] == "true" {
		klog.V(2).Infof("Auto-preservation of failed machines is disabled for machine set %q", machineSet.Name)
		return machines
	}
